...
LIST_MACH_FOLDERS  = [ "Mach0.3" ] # , "Mach1", "Mach5", "Mach10"
...
## ###############################################################
class PlotSimScales():
  def __init__(self, directory_vis):
    self.directory_vis = directory_vis
    WWFnF.createDirectory(self.directory_vis, bool_verbose=False)
    print("Saving figures in:", self.directory_vis)
    print(" ")
    ## INITIALISE DATA CONTAINERS
    ## --------------------------
    if len(LIST_MACH_FOLDERS) == 1:
      self.plot_name = LIST_MACH_FOLDERS[0]
      self.bool_supersonic = LoadData.getNumberFromString(LIST_MACH_FOLDERS[0], "Mach") > 1
    elif len(LIST_MACH_FOLDERS) > 1:
    ...
    for suite_folder in LIST_SUITE_FOLDERS:
      ...
      for sim_folder in LIST_SIM_FOLDERS:
        for mach_folder in LIST_MACH_FOLDERS:
          ...
          directory_sim = WWFnF.createFilepath([
            PATH_SCRATCH, suite_folder, mach_folder, sim_folder
          ])
          if not os.path.isfile(f"{directory_sim}/{FileNames.FILENAME_SIM_SCALES}"):
            print(directory_sim)
            continue
          ## load scales
          print(f"\t> Loading {sim_folder}, {mach_folder} dataset")
          self.__getParams(directory_sim)
  ...
  def __getParams(self, filepath_data):
    ...
    dict_sim_inputs = FlashData.readSimInputs(
  ...
  def plotDependance_knu_Re(self, k_nu_stats_group_sim, domain_name):
    ...
    PlotFuncs.saveFigure(fig, f"{self.directory_vis}/{fig_name}")
    print(" ")
